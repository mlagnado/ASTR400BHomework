...
#  What is the temperature needed to ionize a hydrogen atom (13.6eV)?
# E=3K_BT
T_H = 13.6*u.electronvolt/3/k_B_eV
print(np.round(T_H,3)) #temperature needed to ionize Hydrogen
...
print(np.round(T_recombine,3))


# At what redshift is the temperature sufficient to ionize hydrogen?
z_guess = 19269
print(np.round(Benchmark.Temperature(z_guess),3))


# So why doesn't recombination occur then? - see lecture 
### Many more photons than there are Hydrogen atoms
...
plt.show()
#They are on top of each other so it doesn't depend on other parameters (Omega)
print('## Part 2 ##')
print('# 1 #')
# # Part 2, Question 1
...
HorizonRecombine = Benchmark.ProperDistance(z_recombine, 19269)
print(np.round(HorizonRecombine, 3))


# What angle does the causally connected region of the universe at z=1100 subtend on the sky today?
# theta = size/da
# size = 2*horizon distance
Angle = 2*HorizonRecombine/Benchmark.AngularDiameterDistance(z_recombine)
...
sound_horizon = Benchmark.SoundHorizon(z_recombine,19269)
print(np.round(sound_horizon, 3))

#What is the Sound Horizon TODAY in the Benchmark Cosmology?
sound_horizon_today = sound_horizon*(z_recombine+1)
print(np.round(sound_horizon_today, 3))

#Same for Desitter cosmology
# What is the sound horizon at the epoch of recombination ? 
sound_horizonED = EDesitter.SoundHorizon(z_recombine,19269)
print(np.round(sound_horizonED, 3))

#What is the Sound Horizon TODAY in the DeSitter Cosmology?
sound_horizonED_today = sound_horizonED*(z_recombine+1)
print(np.round(sound_horizonED_today, 3))
...
# What is the angular size subtended by the Sound Horizon Diameter?
ThetaSound = 2*sound_horizon/Benchmark.AngularDiameterDistance(z_recombine)*u.radian.to(u.degree)
print(np.round(ThetaSound, 3))